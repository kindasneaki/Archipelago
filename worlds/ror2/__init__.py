...
class RiskOfRainWorld(World):
    ...
    def create_regions(self) -> None:
        ...
        if self.options.goal == "classic":
            ...
            # explore mode
            create_regions(self, self.options)
...
def create_events(world: MultiWorld, options: ROR2Options, player: int) -> None:
    total_locations = options.total_locations.value
    num_of_events = total_locations // 25
    if total_locations / 25 == num_of_events:
        num_of_events -= 1
    world_region = world.get_region("Petrichor V", player)
    if options.goal == "classic":
